...
You may wish for some characters to start a level with special properties, which you can do by adding fixed events. To do this, create a file called `events.txt` and specify the events you want to add. For instance, `TakesCrownFrom Baron` specifies that Baron starts the level wearing a crown. Events will be ignored if one or more characters involved are not in the level.